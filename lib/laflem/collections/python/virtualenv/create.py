...
class VirtualenvCreateModule(HelloWorldModule):
  ...
  def _main(self, *_args, **kwargs):
    ...
    if (os.path.exists(venv_path) and kwargs['python_virtualenv_create_force']) or kwargs['python_virtualenv_create_remove']:
      console.print(f"Removing [bold red]Virtualenv[/] at [bold green]{venv_path}[/]")
    ...
    if not os.path.exists(venv_path) or kwargs['python_virtualenv_create_force']:
      console.print(f"Creating [bold blue]Virtualenv[/] at [bold green]{venv_path}[/]")
      subprocess.check_call([sys.executable, "-m", "venv", venv_path])
    else:
      console.print(f"[bold red]Virtualenv[/] already exists at [bold green]{venv_path}[/]")

    console.print(f"To activating [bold blue]Virtualenv[/] at [bold green]{venv_path}[/] run :")
