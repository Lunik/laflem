    ...
    for root, dirs, _files in os.walk(os.path.expanduser('~')):
      for directory_name in dirs:
        ...
        if directory_name in VIRTUALENV_FOLDER_NAMES:
          ...
          if Confirm.ask(f"Delete [bold blue]Virtualenv[/] [bold green]{full_path}[/] ?"):
            shutil.rmtree(full_path)
            console.print(f"Deleted [bold blue]Virtualenv[/] at [bold red]{full_path}[/]")
            count_removed += 1

    console.print(f"Removed [bold blue]{count_removed}[/] [bold blue]Virtualenvs[/]")
