...
def install_extra_requirements(extra):
  ...
  answer = Confirm.ask(f"The module require dependecies to run ({','.join(requirements)}). Install them ?")
  if not answer:
    return False
...
def load_git_library():
  ...
  try:
    ...
    if not install_extra_requirements('git'):
      raise ModuleMissingDependency("Unable to run the module without the its dependency.") from error
    return importlib.import_module('git')
